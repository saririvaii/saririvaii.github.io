...
export default function FeaturedProjects() {
    const { data: projects, loading } = useProjects() as {
        data: Project[] | null;
        loading: boolean;
    };
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="flex gap-2">
                                                {[...Array(4)].map((_, j) => (
                                                    <div
                                                        key={j}
                                                        className="h-6 bg-zinc-200 dark:bg-zinc-700 rounded-full w-16"
                                                    ></div>
                                                ))}
                                            </div>
                                        </div>
                            ...
                            return (
                                ...
                                >
                                    <div className="flex flex-col md:flex-row md:gap-8 md:items-center p-6 rounded-xl hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-all duration-500 hover:shadow-lg hover:scale-[1.02]">
                                        {/* Image */}
                                        <div
                                            className={`w-full md:w-1/2 aspect-video bg-zinc-100 dark:bg-zinc-800 relative overflow-hidden rounded-lg group-hover:rounded-xl transition-all duration-300 ${isImageLeft ? "md:order-1" : "md:order-2"}`}
                                        >
                                        ...
                                        {/* Text Content */}
                                        <div
                                            className={`w-full md:w-1/2 flex flex-col items-start mt-4 md:mt-0 ${isImageLeft ? "md:order-2" : "md:order-1"}`}
                                        >
