...
export default function ProjectClient({ project }: ProjectClientProps) {
    ...
    const articleContent = project.content
        ? project.content
              .map((block: any) => {
                                  ...
                                  if (child.marks) {
                                      child.marks.forEach((mark: string) => {
                                          ...
                                          if (mark === "em")
                                              text = `<em>${text}</em>`;
                                          if (mark === "code")
                                              text = `<code class="rounded-md border border-white/10 bg-opacity-5 p-1 text-sm lg:whitespace-nowrap">${text}</code>`;
                                      });
                                  }
                                  return text;
                              })
